...
function copyNarratorText(value: unknown, path: string): string {
  ...
  if (
    ...
    throw new AiReadingError(
      'INVALID_NARRATOR_OUTPUT',
      `${path} must be plain text between 1 and 4,000 characters.`,
    );
  }
  return text;
}
...
function validateAndLabelParagraph(
  ...
  const uncertaintyLabels = [
    ...(candidateDependent ? ['생시 후보에 따라 달라질 수 있습니다.'] : []),
    ...(containsPartialFinding ? ['확인된 기둥 범위의 부분 결과입니다.'] : []),
  ];
  const labeledText =
    uncertaintyLabels.length === 0 ? text : `${uncertaintyLabels.join(' ')} ${text}`;
  if (labeledText.length > 4_100) {
    throw new AiReadingError(
      'INVALID_NARRATOR_OUTPUT',
...
function parseSectionPlan(
  ...
  if (
    ...
    throw new AiReadingError(
      'INVALID_NARRATOR_OUTPUT',
      `${path}.paragraphs must contain 1 through 12 items.`,
    );
  }
  const topic = value.topic as InterpretationTopic;
  const paragraphs = value.paragraphs.map((paragraph, paragraphIndex) =>
    parseParagraphPlan(paragraph, `${path}.paragraphs[${paragraphIndex}]`, findingsById),
  );
...
function parseNarrative(
  ...
  if (!Array.isArray(raw.sections) || raw.sections.length > 15) {
    throw new AiReadingError(
      'INVALID_NARRATOR_OUTPUT',
      'sections must be an array with at most 15 items.',
    );
  }
  const findingsById = new Map(findings.map((finding) => [finding.id, finding] as const));
  const summary = parseParagraphPlan(raw.summary, 'summary', findingsById);
  const sections = raw.sections.map((section, index) =>
    parseSectionPlan(section, index, findingsById),
  );
...
export async function createAiSajuReading(
  ...
  if (request.evidence.findings.length === 0) {
    ...
    narrative = parseNarrative(parsedResponse.output, request.evidence.findings);
  }
