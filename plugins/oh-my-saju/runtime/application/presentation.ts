...
function compactFindingLines(reading: PreparedOhMySajuReading): readonly string[] {
  return packResults(reading).flatMap((result) => {
    ...
    const findingText = detailedFindings.map(
      (finding) =>
        `${finding.id}:${SAJU_TOPIC_TITLES[finding.topic]}=${compactText(
          finding.statement,
        )}${findingQualifier(finding)}`,
    );
    ...
    return [
      `Pack 근거ID|${pack}|${allFindings.map(({ id }) => id).join(',') || '-'}`,
      ...(findingText.length === 0 && unavailableText.length === 0
        ? []
        : [
            `Pack 판정|${compactText(result.interpretation.profile.displayName)}@${
              result.profileRef.version
            }|${[...findingText, ...unavailableText].join(';')}`,
          ]),
    ];
  });
}
