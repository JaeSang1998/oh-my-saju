...
import { isTraditionalSystemError } from '../systems/shared/errors';
import {
...
function isCommandName(value: unknown): value is OhMySajuCommand['command'] {
  return typeof value === 'string' && COMMAND_NAMES.some((name) => name === value);
}

function commandName(value: unknown): OhMySajuCommand['command'] | 'unknown' {
  if (isRecord(value) && isCommandName(value.command)) return value.command;
  return 'unknown';
}
...
function failure(command: unknown, error: unknown): OhMySajuFailure {
  const known =
    ...
    isTraditionalSystemError(error) ||
    isSajuError(error);
  ...
  const path = isSajuError(error) || isTraditionalSystemError(error) ? error.path : undefined;
  const details =
    isOhMySajuApplicationError(error) ||
    isAiReadingError(error) ||
    isSajuInterpretationError(error) ||
...
export async function executeOhMySaju(command: unknown): Promise<OhMySajuResponse> {
  try {
    ...
    throw new OhMySajuApplicationError(
      'INVALID_COMMAND',
      `command must be ${COMMAND_NAMES.join(', ')}.`,
    );
  } catch (error) {
