...
| `plugins/oh-my-saju/runtime/systems` | 택일·토정비결·주역·자미두수·육임의 독립 계산 수직 module |
...
D1  saju-engine: 시간대·절기·원국·생시 후보·구조·timing 사실
 ├→ D2A plugin Traditional systems: 자체 profile/calculator/audit ─────────┐
 └→ D2B plugin Tradition Packs: source/rules/table/helper/provenance       │
      ↓                                                                   │
...
`runtime/systems`는 택일, 토정비결 144괘, 주역 64괘, 자미두수 명반, 육임 사과삼전처럼
서로 다른 입력·중간값·결과 계약을 가진 기능을 각각 하나의 깊은 수직 module로
구현합니다. 공통 계층은 오류·감사 표지·불변 수학 helper만 제공하며, 각 system의
전통 규칙을 하나의 범용 점술 객체로 평탄화하지 않습니다.

직접 TypeScript 함수가 정본 API이고 Application runtime의
`run-traditional-system`은 고정된 registry를 통해 같은 함수를 JSON으로 운반합니다.
호출자가 임의 evaluator나 rule callback을 주입할 수 없고, 모든 결과에는 독립 구현
표지, 적용 profile, 입력 정규화, 명시적 조정, 한계가 포함됩니다. 현대 택일 점수는
재현 가능한 정책값으로 전통 계산 사실과 구분하며, 예측 유효성이 확립되지 않았다는
한계를 숨기지 않습니다.
...
12운성 단계를 구조 관찰로만 내보내며, `sanming-symbolic-curated@1.1.0`은 연지·일간
기준 15개 엄선 신살을 각기 다른 규칙 ID와 출전으로 관측합니다. 생시 미상은 전체
부재가 아닌 부분 관측으로 남기고, 학파 차이가 있는 양인은 두 variant로 보존합니다.
이는 길흉·사건 해석이 아닙니다.
...
검증합니다. Application runtime은 그 과정과 독립 전통 system을 provider-neutral
JSON interface의 세 command로 감쌉니다.
...
run-traditional-system(kind-specific request)
  → package-owned closed registry에서 해당 수직 module 선택
  → 동일한 typed calculator 실행
  → 계산 중간값·근거·profile·한계가 포함된 감사 가능한 결과
```
...
│   ├── systems/                       # election/Tojeong/I Ching/Zi Wei/Liu Ren
│   ├── reading/                       # narration tasks and claim gate
│   └── application/                   # prepare/validate/system command implementation
└── skills/oh-my-saju/
...
자세한 결정은 [ADR 0006](./adr/0006-core-pack-skill-boundaries.md)과
...
- 기존 요청이나 응답의 의미·필수 필드·wire shape를 호환되지 않게 바꾸면 schema와
  package version을 함께 올립니다. 기존 envelope를 보존한 새 명령·선택 필드 추가는
  같은 schema 안의 하위 호환 확장으로 허용하되 package version과 명령별 계약 테스트를
  올립니다.
