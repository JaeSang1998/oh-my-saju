...
<p>
  ...
  <img alt="Oh My Saju plugin 0.4.5" src="https://img.shields.io/badge/plugin-0.4.5-A66A3F" />
...
Oh My Saju는 만세력 계산, 전통 규칙 평가, AI 풀이를 한곳에 모은 TypeScript
도구입니다.

같은 생년월일시라도 절입 시각, 당시의 시간대, 자시 기준, 진태양시 적용 방식에 따라
원국이 달라질 수 있습니다. 이 프로젝트는 어떤 기준을 썼는지 입력값, 중간 계산,
데이터 버전과 함께 돌려줍니다.
...
| `oh-my-saju@0.4.5`  | 개인 사주·두 사람 명식 궁합, 전통 규칙 팩, 택일·토정·주역·자미두수·육임 |
...
```text
1998년 1월 5일 09시 51분 남성과
1996년 5월 27일 06시 50분 여성의 궁합을 봐줘.
```
...
두 사람의 원국을 따로 계산한 뒤 A→B와 B→A의 일간 기준 십신, 두 원국 사이의
천간합과 지지 합·충·형·파·해를 비교합니다. 무엇이 서로를 끌어당기고 어디서
부딪히는지, 오래 지내려면 어떤 점을 맞춰야 하는지 글로 풀이합니다. 생시가
정확하지 않다면 모든 후보 조합에서 같은 내용과 후보마다 달라지는 내용을 나눕니다.

궁합 총점이나 결혼·이별·외도·임신 같은 사건 판정은 만들지 않습니다. 현재는
`ziping-structural-compatibility@1.0.0`의 구조 비교만 지원합니다. 연주·납음·구궁식
등 다른 역사적 궁합법을 추가할 때도 별도 프로필로 구분할 예정입니다.
...
prepare-reading
  ...
  → 기본 풀이는 검증된 문장만 정해진 Markdown 항목에 배치
```

두 사람 궁합은 별도의
`prepare-compatibility → host pair draft → validate-compatibility` 프로토콜을
사용합니다. 두 사람의 개인 풀이를 이어 붙이지 않고 방향별 십신과 교차 관계를 한 쌍의
근거로 검증한 뒤 정해진 궁합 Markdown을 반환합니다.

계산 결과, 규칙 팩의 판정, 모델이 쓴 문장은 각각 다른 필드에 둡니다. 모델이 다른
규칙 팩의 근거를 섞거나 특정 생시 후보에서만 맞는 말을 단정하면 검증 단계에서
걸러집니다.

열린 질문은 `readingMode: "broad"`로 보냅니다. 검증 명령의 `presentationDraft`는
이미 검증된 짧은 문단을 성향·실행·강점/주의점·일/공부·관계 항목에 배치합니다. 각
생활 해석에는 실제 상황과 행동, 그 결과가 드러나야 합니다. `domain`은 문단의 역할,
`direction`은 강점·문제가 생기는 방향·중립 설명을 나타냅니다. 생시 후보에 따라 달라지는 내용에는
`△`, 확인된 기둥만 반영한 내용에는 `◇`를 붙이고 기호 설명은 한 번만 적습니다. 최종
답변은 `result.presentation.markdown`에 들어갑니다. 검증이 끝난 문장을 다시 쓰거나
감사용 notice를 덧붙이지 않습니다.
...
변환기를 직접 사용합니다. KASI 자료는 배포 시점의 회귀 검증에 씁니다.
